...
function drawTextMixin(LeeCanvas) {
    LeeCanvas.prototype.addDrawTextTask = function (text) {
...
function drawTexts(LeeCanvas, _ref) {
    var text = _ref.text,
        style = _ref.style;

    if ((0, _utils.isString)(text)) {
        var textStyle = Object.assign({}, LeeCanvas.fontStyle, style);
        drawText(LeeCanvas, text, textStyle);
    } else if ((0, _utils.isArray)(text)) {
        text.forEach(function (t, index) {
            if ((0, _utils.isString)(t)) {
                var _textStyle = Object.assign({}, LeeCanvas.fontStyle, style);
                drawText(LeeCanvas, t, _textStyle);
                if (index == 0) {
                    delete style.x;
                    delete style.y;
                }
            } else if ((0, _utils.isObject)(t)) {
                var _textStyle2 = Object.assign({}, LeeCanvas.fontStyle, style, t);
                drawText(LeeCanvas, t.text, _textStyle2);
                if (index == 0) {
                    delete style.x;
                    delete style.y;
                }
        ...
        var _textStyle3 = Object.assign({}, LeeCanvas.fontStyle, style, text);
        drawText(LeeCanvas, text.text, _textStyle3);
    }
}
